...
export class LayoutContainerComponent implements OnInit {
	...
	ngOnInit(): void {
		...
		this.eventService
			.on(EventType.CHANGE_LAYOUT_SIZE)
			.subscribe(({ payload }) => {
				setTimeout(() => {
					...
					if (this.layoutSize === LayoutWidth.LAYOUT_WIDTH_BOXED) {
						this.eventService.broadcast(
							...
							SideBarSize.LEFT_SIDEBAR_TYPE_CONDENSED
						);
					} else {
						this.eventService.broadcast(
							EventType.CHANGE_LEFTBAR_SIZE,
							SideBarSize.LEFT_SIDEBAR_TYPE_DEFAULT
						);
					}
				}, 20);
			});
